...
# This is for plot1. Pretty straightforward. 
png(filename = './workspace/plot1.png', width=480, height=480, units = 'px')
with(power.df, hist(Global_active_power, xlab = 'Global Active Power (kilowatt)', main = 'Global Active Power', col='red'))
dev.off()
...
# Taking a step further from plot 3. We now combine four charts in one graphic.
png(filename = './workspace/plot4.png', width=480, height=480, units = 'px')
par(mfrow = c(2, 2))
	# set graphic similar from plot 1. 
	plot(power.df$DateTime, power.df$Global_active_power, type='l', xlab="", ylab="Global Active Power")

	# set graphic similar from plot 2. 
	plot(power.df$DateTime, power.df$Voltage, type='l', xlab='datetime', ylab='Voltage')

	# set graphic similar from plot 3. 
	plot(power.df$DateTime, power.df$Sub_metering_1, xlab="", ylab='Energy sub-metering', type='l')
		lines(power.df$DateTime, power.df$Sub_metering_2, col="red")
		lines(power.df$DateTime, power.df$Sub_metering_3, col="blue")
		legend("topright", legend=c('Sub_metering_1', 'Sub_metering_2', 'Sub_metering_3'), col=c('black', 'red', 'blue'), lwd=1)
	...
	# now for graphic 4. 
	plot(power.df$DateTime, power.df$Global_reactive_power, xlab='datetime', ylab = 'Global_reactive_power', type='l')
dev.off()